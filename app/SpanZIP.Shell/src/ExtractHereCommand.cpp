// SpanZIP.Shell — IExplorerCommand "Extract here" — Sprint 4 scaffold.
//
// Real Invoke spawns `SpanZIP.exe --invoke extract-here --files <paths>`;
...
namespace SpanZIP::Shell
{
    ...
    {
        return SHStrDupW(L"Extract here (SpanZIP)", ppszName);
    }
    ...
    IFACEMETHODIMP ExtractHereCommand::GetState(IShellItemArray* items, BOOL, EXPCMDSTATE* pCmdState) noexcept
    {
        ...
        // show "SpanZIP" in the right-click menu.
