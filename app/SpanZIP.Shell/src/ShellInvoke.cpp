// SpanZIP.Shell — host app launcher.
...
namespace SpanZIP::Shell
{
    ...
    HRESULT InvokeHostApp(IShellItemArray* items, std::wstring_view verb) noexcept
    {
        ...
        // Append `\SpanZIP.exe` — `PathCchCombine` is the modern Path API.
        wchar_t fullExe[MAX_PATH] = {};
        if (FAILED(PathCchCombine(fullExe, MAX_PATH, exePath.c_str(), L"SpanZIP.exe")))
        {
            return E_FAIL;
        }
