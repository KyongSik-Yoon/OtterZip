// OtterZip.Interop — managed Archive facade over the native handle.
//
// Mirrors otterzip_core::Archive but with .NET ergonomics:
...
/// <summary>
/// Managed wrapper around a native <c>OtterzipArchive</c> handle.
...
public sealed class Archive : IDisposable
{
    ...
    private static Archive OpenInternal(string path, string? password)
    {
        ...
        {
            throw new OtterzipException(-1, "otterzip_archive_open returned OK but null handle");
        }
    ...
    public IReadOnlyList<EntryInfo> ReadEntries()
    {
        ...
        try
        {
            ...
            unsafe
            {
                var view = default(OtterzipEntryView);
                while (true)
                {
    ...
    private static unsafe OtterzipExtractOptions BuildDefaultExtractOptions(
        byte* destPtr, int destLen, OverwritePolicy overwrite, bool preserveZoneIdentifier)
    {
        return new OtterzipExtractOptions
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            OtterzipExtractReport reportNative;
            ...
            if (rc == -30 /* OperationCanceled */)
            {
                ...
                throw new OperationCanceledException("otterzip extraction canceled");
            }
            ThrowIfError(rc);
    ...
    ///
    /// Backed by FFI <c>otterzip_archive_test</c> (ABI v5+).
    /// </summary>
    ...
    {
        OtterzipTestReport reportNative;
    ...
    private sealed class ProgressBridge
    {
        ...
        public int OnProgress(in OtterzipProgressView view)
        {
        ...
        public static unsafe int UnmanagedTrampoline(OtterzipProgressView* progress, IntPtr userData)
        {
            try
            {
