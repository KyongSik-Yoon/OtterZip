...
| 대상 | 규칙 | 예 |
|---|---|---|
| 크레이트 | kebab-case | `otterzip-core`, `otterzip-ffi` |
...
- **라이브러리 크레이트(`otterzip-core`, 포맷 백엔드 크레이트):** `thiserror` 필수, `anyhow` **금지**
- **바이너리/통합 테스트:** `anyhow` 허용
- **패닉 금지 영역:** `otterzip-core`, `otterzip-ffi`의 모든 공개 함수
  - `unwrap()`, `expect()`, `panic!`, `unreachable!()` 사용 시 **SAFETY 주석 또는 논리적 근거 주석 필수**
  - `Vec::get(i).unwrap()` 대신 `?` 또는 명시적 에러 리턴
- **FFI 반환 시 오류 매핑:** 내부 `OtterzipError` → `ErrorCode` (i32) + TLS 메시지 저장

```rust
// 예: otterzip-core/src/error.rs
...
```rust
// ✅ 좋음
// SAFETY: `ptr`은 `otterzip_archive_open`이 반환한 유효 포인터이며,
// `otterzip_archive_close` 호출 전까지 살아있음이 호출자 계약(schema.md §4).
...
- **함수:** `otterzip_<namespace>_<verb>` — 예: `otterzip_archive_open`, `otterzip_iterator_next`
- **타입:** `Otterzip<CamelCase>` — 예: `OtterzipArchive`, `OtterzipEntryView`
- **열거형 값:** `OTTERZIP_<NAMESPACE>_<NAME>` — 예: `OTTERZIP_FORMAT_ZIP`, `OTTERZIP_ERROR_WRONG_PASSWORD`
- **콜백 typedef:** `Otterzip<Noun>Cb` — 예: `OtterzipProgressCb`

### 2.2 시그니처 규칙

- **문자열:** 항상 `const char* utf8, size_t len` 쌍. null-terminated에 의존 **금지**
- **출력 핸들:** 마지막 인자 `OtterzipXxx** out_handle`. 호출자가 nullable 체크 후 소유
- **배열:** `T* buf, size_t len` 쌍 + 필요 시 `size_t* out_written`
- **반환값:** 모든 fallible 함수는 `int32_t` 오류 코드. 0 = OK, 음수 = 오류
- **불변 포인터:** `const OtterzipArchive*`로 읽기 전용 명시
- **생명주기:** `_new`/`_free` 또는 `_open`/`_close` 쌍을 항상 함께 제공, 헤더 주석에 소유권 명시

### 2.3 cbindgen 설정

`otterzip-ffi/cbindgen.toml`:

```toml
language = "C"
header = "/* OtterZip C API — Auto-generated by cbindgen. DO NOT EDIT. */"
include_guard = "OTTERZIP_H"
cpp_compat = true
pragma_once = true
style = "type"
documentation = true
...
- C 헤더는 `otterzip-ffi/include/otterzip.h`로 출력, **빌드 아티팩트**로 취급 (Git 체크인하되 수정 금지)
- ABI 호환성 테스트: CI에서 `otterzip_abi_version()` 증가 없이 헤더 diff 발생하면 실패
...
pub extern "C" fn otterzip_archive_close(handle: *mut OtterzipArchive) {
    ...
    // SAFETY: 호출자 계약(§2.4)에 따라 `handle`은 `otterzip_archive_open`이 반환한 값.
...
```rust
#[no_mangle]
pub extern "C" fn otterzip_archive_open(...) -> i32 {
    catch_unwind_to_error(|| -> Result<(), OtterzipError> {
        // 실제 로직
    })
}
```
...
| 대상 | 규칙 | 예 |
|---|---|---|
| 네임스페이스 | PascalCase | `OtterZip.App.Views` |
...
├── SafeHandles/
│   └── ArchiveHandle.cs     // SafeHandle 파생, Close() = otterzip_archive_close
...
{
    private const string Lib = "otterzip";

    [LibraryImport(Lib, EntryPoint = "otterzip_archive_open", StringMarshalling = StringMarshalling.Utf8)]
    ...
    [LibraryImport(Lib, EntryPoint = "otterzip_last_error_message", StringMarshalling = StringMarshalling.Utf8)]
...
public sealed class Archive : IDisposable
{
    ...
    public static Archive Open(string path, ArchiveOpenMode mode, string? password = null)
    {
        int rc = NativeMethods.ArchiveOpen(
            ...
            out IntPtr raw);
        OtterzipException.ThrowIfError(rc);
...
- [ ] 모든 테스트 통과
- [ ] FFI 시그니처 변경 시 `otterzip_abi_version` 증가 + `docs/CHANGELOG-ffi.md` 갱신
...
| 항목 | 위치 | 비고 |
|---|---|---|
| 벤치마크 코퍼스 경로 | `OTTERZIP_BENCH_CORPUS` env | CI·로컬 벤치용 |
| 로그 레벨 | `OTTERZIP_LOG=debug` | `tracing` 필터 |
| 개발자 크래시 덤프 | `OTTERZIP_DUMP_DIR` | 자동 경로 기본값 |

- **사용자 설정은 OS 표준 위치**에 저장 (Windows: `%LOCALAPPDATA%\OtterZip\`, macOS: `~/Library/Application Support/OtterZip/`)
...
OtterZip.Interop/      # 별도 어셈블리 권장 (UI 없는 콘솔 테스트용)
```
