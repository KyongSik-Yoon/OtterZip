...
- **看起来像 Windows，因为它就是** — 由 C# 与 WinUI 3 构建。真正的原生界面，而不是套在窗口里的网页。 在 Linux 上，则是构建在同一引擎之上的原生 Avalonia 窗口。
...
下载 `linux-x64` tarball，解压后运行 `./install.sh` — 安装到 `~/.local`，无需 root 权限。可在 **设置 → 集成** 中把 OtterZip 添加到文件管理器的右键菜单。

包括命令行用法以及与 Windows 的差异在内的完整说明：[**Linux 上的 OtterZip**](LINUX.md)。
...
**Windows** — Windows 10（版本 2004）或更高 · x64

**Linux** — x64 或 arm64，X11 或 Wayland。发布的 tarball 是自包含的，无需预先安装运行时。
