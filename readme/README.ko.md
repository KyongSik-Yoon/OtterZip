...
- **Windows처럼 보이는 이유는, 실제로 그렇기 때문** — C#과 WinUI 3으로 만들었습니다. 창 안에 띄운 웹페이지가 아니라 진짜 네이티브 인터페이스입니다. Linux에서는 같은 엔진 위에 올린 네이티브 Avalonia 창입니다.
...
`linux-x64` tarball을 내려받아 압축을 풀고 `./install.sh`를 실행하세요 — `~/.local`에 설치되며 root 권한이 필요 없습니다. 파일 관리자의 우클릭 메뉴에는 **설정 → 통합**에서 추가할 수 있습니다.

명령줄 사용법과 Windows와 다른 점을 포함한 자세한 내용: [**Linux의 OtterZip**](LINUX.md).
...
**Windows** — Windows 10 (버전 2004) 이상 · x64

**Linux** — x64 또는 arm64, X11 또는 Wayland. 릴리스 tarball은 자체 포함형이라 런타임을 따로 설치할 필요가 없습니다.
