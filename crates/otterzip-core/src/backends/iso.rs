...
//! [`OtterzipError::UnsupportedFormat`] with a "UDF" hint string so
...
impl IsoBackend {
    pub(crate) fn open(path: &Path) -> Result<Self> {
        let mut block_io = FileBlockIo::open(path).map_err(OtterzipError::Io)?;
...
fn map_iso_err(e: Iso9660Error) -> OtterzipError {
    match e {
        Iso9660Error::IoError | Iso9660Error::ReadFailed => {
            OtterzipError::Io(io::Error::new(io::ErrorKind::Other, "iso9660 io"))
        }
        ...
        | Iso9660Error::NotFound => OtterzipError::UnsupportedFormat(Some(
            "ISO9660 volume descriptor missing — UDF? (v1.1)".into(),
        )),
        other => OtterzipError::BackendError(format!("iso9660: {other:?}")),
    }
}
