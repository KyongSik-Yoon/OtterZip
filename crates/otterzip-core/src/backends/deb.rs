...
//! the package contents can re-open `data.tar.xz` with OtterZip.
//!
...
impl DebBackend {
    pub(crate) fn open(path: &Path) -> Result<Self> {
        let mut f = BufReader::new(File::open(path).map_err(OtterzipError::Io)?);
        ...
        let mut magic = [0u8; 8];
        f.read_exact(&mut magic).map_err(OtterzipError::Io)?;
        if &magic != AR_MAGIC {
            return Err(OtterzipError::UnsupportedFormat(Some(
                "deb: missing `!<arch>\\n` magic".into(),
            )));
        }

        let total_len = f.get_ref().metadata().map_err(OtterzipError::Io)?.len();
        ...
        while pos + AR_HEADER_LEN <= total_len {
            f.seek(SeekFrom::Start(pos)).map_err(OtterzipError::Io)?;
            let mut header = [0u8; 60];
            f.read_exact(&mut header).map_err(OtterzipError::Io)?;
            ...
            let size_str = std::str::from_utf8(&header[48..58])
                .map_err(|_| OtterzipError::BackendError("deb: non-ASCII size field".into()))?;
            let size: u64 = size_str
                .trim()
                .parse()
                .map_err(|_| OtterzipError::BackendError("deb: malformed size field".into()))?;
...
impl ArchiveBackend for DebBackend {
    ...
    fn extract_entry(&self, entry_path: &str, out: &mut dyn std::io::Write) -> Result<u64> {
        ...
        let mut f = File::open(&self.path).map_err(OtterzipError::Io)?;
