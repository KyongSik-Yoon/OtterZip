...
//! via `otterzip_core::Archive`, enumerate entries, extract, verify bytes.
//!
...
use otterzip_core::{Archive, ArchiveFormat, ExtractOptions, OpenMode, OverwritePolicy};
use tempfile::tempdir;
use zip::write::SimpleFileOptions;

const PAYLOAD_A: &[u8] = b"hello otterzip\n";
...
fn format_detection_recognises_our_fixture() {
    ...
    let fmt = otterzip_core::detect(&zip_path).unwrap();
...
fn extract_all_writes_files_with_correct_content() {
    ...
    let archive = Archive::open(&zip_path, OpenMode::Read).unwrap();
    let report = archive.extract_all::<fn(&otterzip_core::Progress) -> bool>(&opts, None).unwrap();
...
fn truly_unsupported_format_rejected() {
    ...
    match err {
        otterzip_core::OtterzipError::UnsupportedFormat(_) => {}
