//! Regression (claim 3): the `pax_global_header` pseudo-entry that `git archive`
//! puts at the head of every tarball used to extract as a real junk file and
//! poison Smart Extract's root-layout detection. It is now filtered from every
//! tar walk (is_pax_pseudo_entry).
//!
