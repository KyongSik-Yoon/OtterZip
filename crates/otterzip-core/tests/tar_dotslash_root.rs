//! Regression (fb5449b): `tar -cf x.tar -C dir .` emits a leading `./` directory
//! member that resolves to dest_root itself. The d25b3e4 traversal hardening
//! briefly read that as an escape and aborted the whole extraction; the fix
//! accepts a DIRECTORY entry resolving to dest_root (only a FILE there is
//! rejected). Every `tar -C <dir> .` tarball in existence starts this way.
//!
