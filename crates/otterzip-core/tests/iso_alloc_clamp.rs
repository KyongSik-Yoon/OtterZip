//! Regression: the ISO backend used to allocate the directory record's declared
//! `data_length` (unvalidated u32) before reading any bytes, so a tiny crafted
//! .iso could force a multi-GiB commit. extract_entry now clamps the buffer to
//! the image's physical extent.
//!
