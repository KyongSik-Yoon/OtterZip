//! T1-4 (extract side): `ExtractOptions::preserve_timestamps` (default true)
//! must carry the archived mtime onto every extracted file, across both the
//! ZIP extractor and the hand-rolled streaming tar extractor. Regression guard
//! for the `__apply_extract_mtime` wire — the field used to be inert, stamping
//! every file with wall-clock "now" (a silent data loss on backup restore).
