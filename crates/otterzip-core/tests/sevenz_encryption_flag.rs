//! Regression: `backends/sevenz.rs::meta_to_entry` used to hard-code
//! `encryption: EncryptionMethod::None`, so no 7z entry ever reported
//! encryption and `Archive::is_encrypted()` was false for every 7z. It now
//! reads the AES-256 coder from the entry's block via `stream_map`.
//!
