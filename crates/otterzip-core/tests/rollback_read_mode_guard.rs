//! Regression: `rollback()` on a READ-mode `Archive` used to delete the source
//! archive off disk (no mode guard, unlike `commit()`). It now rejects a read
//! handle before removing anything.
