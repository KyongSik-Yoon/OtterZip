//! Regression: RAR5 archives hashed with BLAKE2sp (WinRAR `-htb`) used to be
//! reported as corrupt by Archive::test.
//!
...
fn rar5_blake2sp_entry_passes_verification() {
    ...
    // The BLAKE2sp entry's crc32 field STILL surfaces the digest's first 4
    // bytes: unrar 0.5.8 exposes the HashValue union with no type discriminator,
    // so a real CRC32 and a BLAKE2sp prefix are indistinguishable at the
    // metadata level. That residual is cosmetic and unfixable from here — the
    // fix is that Archive::test no longer TRUSTS this field for RAR, relying on
    // unrar's own hash verification during extraction instead.
    println!(
        "blake crc32 field = {blake_field:?} (digest prefix 0x{BLAKE_PREFIX_AS_U32:08x}, \
         unreliable for RAR — not compared)"
    );
    assert_eq!(
        blake_corrupt, 0,
        "a valid -htb (BLAKE2sp) RAR5 must not report entries as corrupted; \
         test() must rely on unrar's hash check, not the raw crc32 field"
    );
}
