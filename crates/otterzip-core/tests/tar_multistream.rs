//! Regression (claim 5): `.tar.gz` / `.tar.bz2` / `.tar.xz` built from MULTIPLE
//! concatenated compressed streams used to drop every member past the first
//! stream. The backend now uses the Multi* decoders.
//!
