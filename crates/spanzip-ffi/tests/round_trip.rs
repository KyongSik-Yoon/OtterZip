...
//! exported `spanzip_archive_open`, iterates entries via the C iterator,
...
use spanzip_ffi::{
    spanzip_archive_add_directory, spanzip_archive_close, spanzip_archive_commit,
    spanzip_archive_create, spanzip_archive_extract_all, spanzip_archive_format,
    spanzip_archive_open, spanzip_detect_format, spanzip_iterator_free, spanzip_iterator_new,
    spanzip_iterator_next, SpanzipArchive, SpanzipCreateOptions, SpanzipEntryView,
    SpanzipExtractOptions, SpanzipExtractReport, SpanzipIterator, SpanzipProgressView,
};

const SPANZIP_OK: i32 = 0;
const SPANZIP_ERR_ITERATOR_END: i32 = -25;
...
fn detect_format_via_ffi() {
    ...
    let mut fmt: u32 = 0;
    let rc = spanzip_detect_format(buf.as_ptr().cast(), len, &mut fmt);
    assert_eq!(rc, SPANZIP_OK);
...
fn open_iterate_close_via_ffi() {
    ...
    let (buf, len) = path_arg(&zip_path);
    let mut handle: *mut SpanzipArchive = ptr::null_mut();
    let rc = spanzip_archive_open(
        buf.as_ptr().cast(),
        len,
        0, // OpenMode::Read
        ptr::null(),
    ...
    let mut fmt: u32 = 0;
    assert_eq!(spanzip_archive_format(handle, &mut fmt), SPANZIP_OK);
    assert_eq!(fmt, FORMAT_ZIP);

    let mut iter: *mut SpanzipIterator = ptr::null_mut();
    assert_eq!(spanzip_iterator_new(handle, &mut iter), SPANZIP_OK);
    assert!(!iter.is_null());

    let mut names = Vec::new();
    let mut view = SpanzipEntryView::default();
    loop {
        let rc = spanzip_iterator_next(iter, &mut view);
        if rc == SPANZIP_ERR_ITERATOR_END {
            break;
        }
        assert_eq!(rc, SPANZIP_OK);
    ...
    spanzip_iterator_free(iter);
    spanzip_archive_close(handle);
}

extern "C" fn count_progress_ticks(
    _view: *const SpanzipProgressView,
    user: *mut c_void,
) -> i32 {
    if !user.is_null() {
...
fn extract_all_via_ffi_with_progress_callback() {
    ...
    let opts = SpanzipExtractOptions {
    ...
    let rc = spanzip_archive_extract_all(
        handle,
        &opts,
...
fn create_then_extract_via_ffi() {
    ...
    let rc = spanzip_archive_add_directory(
        handle,
        src_buf.as_ptr().cast(),
        src_len,
        ptr::null(),
    ...
    let rc = spanzip_archive_commit(handle);
    assert_eq!(rc, SPANZIP_OK);

    // Now re-open via FFI and verify content.
    assert!(archive_path.exists());
    let (re_path, re_len) = path_arg(&archive_path);
    let mut read_handle: *mut SpanzipArchive = ptr::null_mut();
    let rc = spanzip_archive_open(
        re_path.as_ptr().cast(),
        re_len,
        0,
    ...
    assert_eq!(rc, SPANZIP_OK);
    let mut fmt = 0u32;
    assert_eq!(spanzip_archive_format(read_handle, &mut fmt), SPANZIP_OK);
    assert_eq!(fmt, FORMAT_ZIP);
    spanzip_archive_close(read_handle);
}
