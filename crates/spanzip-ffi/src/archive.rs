...
use spanzip_core::{Archive, ArchiveFormat, OpenMode, SpanzipError};
...
#[repr(C)]
pub struct SpanzipArchive {
    _private: [u8; 0],
}

fn open_mode_from_u32(value: u32) -> Result<OpenMode, SpanzipError> {
    match value {
        0 => Ok(OpenMode::Read),
        1 => Ok(OpenMode::CreateNew),
        2 => Ok(OpenMode::CreateOrOverwrite),
        3 => Ok(OpenMode::Update),
        _ => Err(SpanzipError::InvalidArgument("mode out of range")),
    }
}
...
#[no_mangle]
pub extern "C" fn spanzip_detect_format(
...
#[no_mangle]
pub extern "C" fn spanzip_detect_format_bytes(
    data: *const u8,
    len: usize,
    out_format: *mut u32,
) -> i32 {
    catch_unwind_to_error(|| {
        ...
        if data.is_null() && len > 0 {
            return Err(SpanzipError::InvalidArgument("null data with non-zero len"));
        }
        // SAFETY: caller-provided slice; len validated against null above.
        let bytes = unsafe { std::slice::from_raw_parts(data, len) };
        let fmt = spanzip_core::detect_bytes(bytes).unwrap_or(ArchiveFormat::Unknown);
...
#[no_mangle]
pub extern "C" fn spanzip_archive_entry_count(
    handle: *const SpanzipArchive,
    out_count: *mut u64,
) -> i32 {
    catch_unwind_to_error(|| {
        if handle.is_null() {
        ...
        if out_count.is_null() {
            return Err(SpanzipError::InvalidArgument("out_count is null"));
        }
        // SAFETY: handle from `spanzip_archive_open`.
...
#[no_mangle]
pub extern "C" fn spanzip_archive_is_encrypted(
    handle: *const SpanzipArchive,
    out_bool: *mut u8,
) -> i32 {
    catch_unwind_to_error(|| {
        ...
        if out_bool.is_null() {
            return Err(SpanzipError::InvalidArgument("out_bool is null"));
        }
...
#[no_mangle]
pub extern "C" fn spanzip_archive_is_solid(
    handle: *const SpanzipArchive,
    out_tri: *mut i8,
) -> i32 {
    catch_unwind_to_error(|| {
        ...
        if out_tri.is_null() {
            return Err(SpanzipError::InvalidArgument("out_tri is null"));
        }
        // SAFETY: handle valid per open contract.
        let archive = unsafe { &*handle.cast::<Archive>() };
...
#[no_mangle]
pub extern "C" fn spanzip_archive_open(
    path_utf8: *const c_char,
    path_len: usize,
    mode: u32,
    password_utf8: *const c_char,
    password_len: usize,
    out_handle: *mut *mut SpanzipArchive,
) -> i32 {
    catch_unwind_to_error(|| {
        if out_handle.is_null() {
            return Err(SpanzipError::InvalidArgument("out_handle is null"));
        }
        ...
        // owned by C; `spanzip_archive_close` must be called to release.
        unsafe {
            *out_handle = Box::into_raw(boxed).cast::<SpanzipArchive>();
        }
        Ok(OK)
    })
}
...
pub extern "C" fn spanzip_archive_close(handle: *mut SpanzipArchive) {
    ...
    // SAFETY: handle came from `spanzip_archive_open`, which boxed an
...
/// if `handle` is NULL.
#[no_mangle]
pub extern "C" fn spanzip_archive_format(
    handle: *const SpanzipArchive,
    out_format: *mut u32,
) -> i32 {
    catch_unwind_to_error(|| {
...
/// [`spanzip_archive_open`] (or a future create/update equivalent), and the
/// caller must enforce single-threaded access per `ffi-api.md` §0.1 #7.
pub(crate) unsafe fn archive_ref<'a>(handle: *const SpanzipArchive) -> Option<&'a Archive> {
    if handle.is_null() {
        return None;
    }
