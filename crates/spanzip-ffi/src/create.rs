...
use spanzip_core::format::{ArchiveFormat, CompressionMethod, EncryptionMethod};
use spanzip_core::{Archive, CreateOptions, SpanzipError};
use zeroize::Zeroizing;

use crate::archive::SpanzipArchive;
...
#[repr(C)]
pub struct SpanzipCreateOptions {
    pub format: u32,
    pub compression: u32,
...
fn format_from_u32(v: u32) -> Result<ArchiveFormat, SpanzipError> {
    match v {
        ...
        _ => Err(SpanzipError::InvalidArgument("format out of range")),
    }
}

fn compression_from_u32(v: u32) -> Result<CompressionMethod, SpanzipError> {
    match v {
        ...
        _ => Err(SpanzipError::InvalidArgument("compression out of range")),
    }
}

fn encryption_from_u32(v: u32) -> Result<EncryptionMethod, SpanzipError> {
    match v {
        0 => Ok(EncryptionMethod::None),
        1 => Ok(EncryptionMethod::ZipCrypto),
        2 => Ok(EncryptionMethod::Aes128),
        3 => Ok(EncryptionMethod::Aes256),
        _ => Err(SpanzipError::InvalidArgument("encryption out of range")),
    }
}
...
#[no_mangle]
pub extern "C" fn spanzip_archive_create(
    path_utf8: *const c_char,
    path_len: usize,
    opts: *const SpanzipCreateOptions,
...
#[no_mangle]
pub extern "C" fn spanzip_archive_add_file(
    ...
    catch_unwind_to_error(|| {
        ...
        // SAFETY: handle from `spanzip_archive_create`. Single-threaded use.
...
#[no_mangle]
pub extern "C" fn spanzip_archive_add_directory(
    handle: *mut SpanzipArchive,
    ...
    catch_unwind_to_error(|| {
        ...
        // SAFETY: handle from `spanzip_archive_create`.
        let archive = unsafe { &mut *handle.cast::<Archive>() };
        archive.add_dir_recursive::<fn(&spanzip_core::Progress) -> bool>(
            PathBuf::from(src),
            prefix,
            None,
        )?;
...
/// it afterwards).
#[no_mangle]
pub extern "C" fn spanzip_archive_commit(handle: *mut SpanzipArchive) -> i32 {
    catch_unwind_to_error(|| {
        if handle.is_null() {
            return Ok(ErrorCode::InvalidHandle as i32);
        }
        // SAFETY: handle from `spanzip_archive_create`. We `Box::from_raw`
...
#[no_mangle]
pub extern "C" fn spanzip_archive_rollback(handle: *mut SpanzipArchive) -> i32 {
    catch_unwind_to_error(|| {
        if handle.is_null() {
            return Ok(ErrorCode::InvalidHandle as i32);
        }
