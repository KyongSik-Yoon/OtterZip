...
//!
//! `spanzip_reader_open` returns an opaque `SpanzipReader*` that wraps
//! the result of `Archive::read_entry`. `spanzip_reader_read` pulls
...
//! to keep the parent `SpanzipArchive*` alive at least as long as the
...
#[no_mangle]
pub extern "C" fn spanzip_reader_open(
    handle: *mut SpanzipArchive,
    entry_path_utf8: *const c_char,
    path_len: usize,
    out_reader: *mut *mut SpanzipReader,
) -> i32 {
    catch_unwind_to_error(|| {
        if out_reader.is_null() {
            return Err(SpanzipError::InvalidArgument("out_reader is null"));
        }
        // SAFETY: handle from `spanzip_archive_open`. archive_ref does null check.
        ...
        // `spanzip_reader_close` returns; this matches the iterator
        ...
        unsafe { *out_reader = Box::into_raw(state).cast::<SpanzipReader>() };
        Ok(OK)
    })
}
...
#[no_mangle]
pub extern "C" fn spanzip_reader_read(
    reader: *mut SpanzipReader,
    buffer: *mut u8,
    buffer_len: usize,
) -> i64 {
    let result = catch_unwind_to_error(|| {
        ...
        if buffer.is_null() && buffer_len > 0 {
            return Err(SpanzipError::InvalidArgument(
...
#[no_mangle]
pub extern "C" fn spanzip_reader_close(reader: *mut SpanzipReader) {
    if reader.is_null() {
        return;
    }
    // SAFETY: came from `spanzip_reader_open`.
    unsafe {
