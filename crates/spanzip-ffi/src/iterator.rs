...
use spanzip_core::{Entry, EntryIter, SpanzipError};
...
struct IterState {
    /// Erased lifetime: see SAFETY in `spanzip_iterator_new`.
...
fn fill_view(view: &mut SpanzipEntryView, entry: &Entry, state: &mut IterState) {
    ...
    view.is_encrypted = u8::from(entry.encryption != spanzip_core::EncryptionMethod::None);
    view._reserved = 0;
...
#[no_mangle]
pub extern "C" fn spanzip_iterator_new(
    handle: *const SpanzipArchive,
    out_iter: *mut *mut SpanzipIterator,
) -> i32 {
    catch_unwind_to_error(|| {
        if out_iter.is_null() {
            return Err(SpanzipError::InvalidArgument("out_iter is null"));
        }
        ...
        unsafe {
            *out_iter = Box::into_raw(state).cast::<SpanzipIterator>();
        }
        Ok(OK)
    })
}
...
#[no_mangle]
pub extern "C" fn spanzip_iterator_next(
    iter: *mut SpanzipIterator,
    out_entry: *mut SpanzipEntryView,
) -> i32 {
    catch_unwind_to_error(|| {
        if iter.is_null() {
        ...
        if out_entry.is_null() {
            return Err(SpanzipError::InvalidArgument("out_entry is null"));
        }
        // SAFETY: iter came from `spanzip_iterator_new`, single-threaded use.
...
#[no_mangle]
pub extern "C" fn spanzip_iterator_free(iter: *mut SpanzipIterator) {
    if iter.is_null() {
        return;
    }
    // SAFETY: came from `spanzip_iterator_new`.
    unsafe {
