...
//!
//! Exposes `spanzip_archive_test` (drives `Archive::test`) and
//! `spanzip_archive_test_corrupted_entry` (look up the i-th corrupted
...
use spanzip_core::{Progress, ProgressSink, SpanzipError, TestReport};

use crate::archive::{archive_ref, SpanzipArchive};
use crate::error::{ErrorCode, OK};
use crate::extract::{SpanzipProgressCb, SpanzipProgressView, SpanzipTestReport};
use crate::util::catch_unwind_to_error;

thread_local! {
    /// Most recent test result. Holds null-terminated copies so
    /// `spanzip_archive_test_corrupted_entry` can hand back stable
...
#[no_mangle]
pub extern "C" fn spanzip_archive_test(
    handle: *const SpanzipArchive,
    progress_cb: SpanzipProgressCb,
    user_data: *mut c_void,
    out_report: *mut SpanzipTestReport,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn spanzip_archive_test_corrupted_entry(
    _handle: *const SpanzipArchive,
    index: u32,
