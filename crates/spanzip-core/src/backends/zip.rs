        ...
        match outcome {
            Outcome::Plain => archive.by_name(name).map_err(|e| match e {
                zip::result::ZipError::FileNotFound => {
                    SpanzipError::EntryNotFound(name.to_string())
                }
            ...
            Outcome::NotFound => Err(SpanzipError::EntryNotFound(name.to_string())),
...
impl ZipBackend {
    ...
    fn extract_all_parallel(&self, ctx: &mut StreamingExtractCtx<'_>) -> Result<()> {
        ...
        let first_err: Mutex<Option<SpanzipError>> = Mutex::new(None);
        ...
        metas.par_iter().for_each_init(
                ...
                Err(orig) => {
                    if opts.block_path_traversal {
                        set_first_err(
                            &first_err,
                            &canceled,
                            SpanzipError::PathTraversalBlocked(orig),
                        );
                    } else {
            ...
            if entry.is_directory {
                if let Err(e) = std::fs::create_dir_all(&out_path) {
                    set_first_err(&first_err, &canceled, SpanzipError::Io(e));
                    return;
                }
                ...
                match opts.overwrite {
                    OverwritePolicy::Never => {
                        set_first_err(
                            &first_err,
                            &canceled,
                            SpanzipError::Io(std::io::Error::new(
        ...
        if canceled.load(std::sync::atomic::Ordering::Relaxed) {
            return Err(SpanzipError::Canceled);
        }
...
fn set_first_err(
    slot: &Mutex<Option<SpanzipError>>,
...
fn map_zip_err(e: zip::result::ZipError) -> SpanzipError {
    ...
    match e {
        Z::Io(io) => SpanzipError::Io(io),
        Z::InvalidArchive(msg) => SpanzipError::Corrupted {
            reason: msg.to_string(),
            entry: None,
        },
        Z::UnsupportedArchive(msg) => SpanzipError::UnsupportedFormat(Some(msg.to_string())),
        Z::FileNotFound => SpanzipError::EntryNotFound(String::new()),
