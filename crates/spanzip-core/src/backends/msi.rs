...
//!   with SpanZIP if they want the inner files. Auto-flattening
...
fn open_compound(path: &Path) -> Result<CompoundFile<BufReader<File>>> {
    let f = File::open(path).map_err(SpanzipError::Io)?;
    cfb::CompoundFile::open(BufReader::new(f)).map_err(|e| {
        ...
        if e.kind() == io::ErrorKind::InvalidData {
            SpanzipError::UnsupportedFormat(Some(format!("msi: {e}")))
        } else {
            SpanzipError::BackendError(format!("msi open: {e}"))
        }
    })
}

/// CFB uses backslash as the path separator. Translate to forward
/// slash to match the rest of SpanZIP (and trim the leading slash
...
impl ArchiveBackend for MsiBackend {
    ...
    fn extract_entry(&self, entry_path: &str, out: &mut dyn std::io::Write) -> Result<u64> {
        ...
        let mut stream = comp
            ...
            .map_err(|e| SpanzipError::BackendError(format!("msi open_stream: {e}")))?;
