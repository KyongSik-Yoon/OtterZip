...
//! `open_multi`, and mutation APIs return `SpanzipError::UnsupportedFormat`
...
impl Archive {
    ...
    /// or [`Archive::entries`] on it returns [`SpanzipError::InvalidArgument`].
    ...
    pub fn commit(mut self) -> Result<()> {
        match std::mem::replace(
            ...
            ArchiveInner::Reader(_) => Err(SpanzipError::InvalidArgument(
    ...
    fn reader(&self) -> Result<&(dyn ArchiveBackend + Send)> {
        match &self.inner {
            ...
            ArchiveInner::Writer(_) => Err(SpanzipError::InvalidArgument(
    ...
    pub fn extract_all<S: ProgressSink>(
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            if let Some(err) = check_zip_bomb(entry, opts) {
                if let SpanzipError::ZipBombSuspected { entry: name, ratio, limit } = &err {
                    tracing::warn!(
            ...
            let out_path = match resolve_output_path(&dest_root, &entry.path, opts) {
                ...
                Err(Skipped::Traversal(orig)) => {
                    if opts.block_path_traversal {
                        ...
                        return Err(SpanzipError::PathTraversalBlocked(orig));
                    }
                    tracing::warn!(
                        target: "spanzip::security",
            ...
            if let Err(err) = bomb_monitor.record(written, entry.compressed_size) {
                if let SpanzipError::ZipBombSuspected { ratio, limit, .. } = &err {
                    tracing::warn!(
                        target: "spanzip::security",
...
impl ArchiveBackend for EmptyBackend {
    fn entries(&self) -> Result<Box<dyn Iterator<Item = Result<crate::Entry>> + '_>> {
        Err(SpanzipError::InvalidArgument(
...
impl __BombMonitor {
    ...
    pub fn record(
        ...
        uncompressed: u64,
        compressed: u64,
    ) -> std::result::Result<(), SpanzipError> {
        ...
        if self.max_total_bytes > 0 && self.uncompressed_total > self.max_total_bytes {
            return Err(SpanzipError::ZipBombSuspected {
...
fn check_zip_bomb(
    ...
    if ratio > u64::from(opts.max_compression_ratio) {
        Some(SpanzipError::ZipBombSuspected {
