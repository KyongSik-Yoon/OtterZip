...
fn iso9660_minimal_round_trip() {
    let payload_a: &[u8] = b"hello world from spanzip iso backend\n";
...
fn iso_extension_hint_routes_to_iso_variant() {
    ...
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Iso);

    let img = td.path().join("anything.img");
    fs::write(&img, b"").unwrap();
    assert_eq!(spanzip_core::detect(&img).unwrap(), ArchiveFormat::Iso);
}
...
fn random_bytes_in_iso_extension_yield_unsupported_format() {
    ...
    match result {
        Err(SpanzipError::UnsupportedFormat(hint)) => {
        ...
        Err(SpanzipError::BackendError(msg)) => {
