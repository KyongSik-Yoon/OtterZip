...
//! benchmark harness. The criterion bench in `crates/spanzip-bench`
...
use spanzip_core::{Archive, ArchiveFormat, ExtractOptions, OpenMode, ProgressSink};
...
fn fixture_payload() -> Vec<u8> {
    ...
    for i in 0..16 * 1024u32 {
        v.push((i & 0xFF) as u8);
        if i % 17 == 0 {
            v.extend_from_slice(b"SpanZIP F2 ZSTD/LZ4 acceptance ");
        }
    }
    v
}
...
fn detect_routes_zst_to_zstd_variant() {
    ...
    write_zstd(&src, &payload);
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Zstd);
}
...
fn detect_routes_tar_zst_via_double_extension() {
    ...
    write_tar_zst(&src, members);
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::TarZst);
}
...
fn detect_routes_lz4_to_lz4_variant() {
    ...
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Lz4);
}
...
fn detect_routes_tar_lz4_via_double_extension() {
    ...
    write_tar_lz4(&src, members);
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::TarLz4);
}
