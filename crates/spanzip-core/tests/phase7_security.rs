...
use spanzip_core::{
    Archive, ExtractOptions, OpenMode, OverwritePolicy, Progress, SpanzipError,
};
...
fn extract_with_defaults(zip: &std::path::Path, dest: &std::path::Path) -> spanzip_core::Result<()> {
...
fn rejects_ntfs_alternate_data_stream() {
    ...
    assert!(
        matches!(err, SpanzipError::PathTraversalBlocked(_)),
