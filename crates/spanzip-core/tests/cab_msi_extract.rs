...
fn cab_magic_routes_to_cab_variant() {
    ...
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Cab);
}
...
fn msi_extension_routes_to_msi_variant() {
    ...
    fs::write(&src, b"").unwrap();
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Msi);
}
...
fn ole2_magic_with_non_msi_extension_falls_back_to_unknown() {
    ...
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Unknown);
}
