...
use spanzip_core::{
    Archive, ArchiveFormat, ExtractOptions, OpenMode, OverwritePolicy, SpanzipError,
};
...
fn targz_format_detected() {
    ...
    let fmt = spanzip_core::detect(&path).unwrap();
...
fn targz_path_traversal_blocked() {
    ...
    let err = archive
        .extract_all::<fn(&spanzip_core::Progress) -> bool>(&opts, None)
        .unwrap_err();
    assert!(matches!(err, SpanzipError::PathTraversalBlocked(_)));
...
fn encrypted_zip_requires_password() {
    ...
    let err = archive
        .extract_all::<fn(&spanzip_core::Progress) -> bool>(&opts, None)
        .unwrap_err();
    assert!(matches!(err, SpanzipError::WrongPassword), "got {err:?}");
}
...
fn zip_bomb_blocked_by_default_threshold() {
    ...
    let err = archive
        ...
        .unwrap_err();
    assert!(
        matches!(err, SpanzipError::ZipBombSuspected { .. }),
