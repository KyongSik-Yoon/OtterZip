...
use spanzip_core::{
    Archive, ArchiveFormat, ExtractOptions, OpenMode, ProgressSink, SpanzipError,
};
...
fn deb_round_trip_three_members() {
    ...
    let control_tar_gz = make_data_tar_gz(&[("control", b"Package: spanzip-test\n")]);
...
fn deb_extension_routes_to_deb_variant() {
    ...
    assert_eq!(spanzip_core::detect(&src).unwrap(), ArchiveFormat::Deb);
}
...
fn ar_magic_routes_to_deb_only_when_extension_agrees() {
    ...
    fs::write(&deb, b"!<arch>\nrest").unwrap();
    assert_eq!(spanzip_core::detect(&deb).unwrap(), ArchiveFormat::Deb);
    ...
    assert_eq!(spanzip_core::detect(&staticlib).unwrap(), ArchiveFormat::Unknown);
}
...
fn missing_magic_yields_unsupported() {
    ...
    let result = Archive::open(&src, OpenMode::Read);
    match result {
        Err(SpanzipError::UnsupportedFormat(_)) | Err(SpanzipError::BackendError(_)) => {}
